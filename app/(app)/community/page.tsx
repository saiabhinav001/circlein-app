          ...
          <TabsContent value="polls" className="space-y-4">
            ...
            {isAdmin && (
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="grid md:grid-cols-3 gap-2">
                    <Button type="button" variant="outline" className="justify-start" onClick={() => applyPollTemplate('hours')}>
                      <Wand2 className="w-4 h-4 mr-2" /> Hours template
                    </Button>
                    <Button type="button" variant="outline" className="justify-start" onClick={() => applyPollTemplate('event')}>
                      <Wand2 className="w-4 h-4 mr-2" /> Event template
                    </Button>
                    <Button type="button" variant="outline" className="justify-start" onClick={() => applyPollTemplate('facility')}>
                      <Wand2 className="w-4 h-4 mr-2" /> Facility template
                    </Button>
                  </div>
            ...
            {polls.length === 0 ? (
              ...
              polls.map((poll) => {
                const totalVotes = Object.keys(poll.votes).length;
                const hasVoted = poll.votes[userEmail] !== undefined;
                const isClosed = poll.status !== 'open' || poll.deadline.getTime() <= nowTick;
                  ...
                  <Card key={poll.id}>
                    <CardHeader className="pb-3">
                      ...
                      <CardDescription className="flex items-center gap-2">
                        <Clock className="w-3.5 h-3.5" /> {formatCountdown(poll.deadline)} • {totalVotes} votes
                      </CardDescription>
                    </CardHeader>
                    ...
                    <CardContent className="space-y-3">
                      ...
                      <div className="h-52 w-full overflow-x-auto">
                        <div className="h-full min-w-[480px] sm:min-w-0">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={pollChartData(poll)}>
                              <CartesianGrid strokeDasharray="3 3" />
                              <XAxis dataKey="name" tick={{ fontSize: 12 }} />
                              <YAxis allowDecimals={false} />
                              <Tooltip />
                              <Bar dataKey="votes" fill="#1f2937" radius={[6, 6, 0, 0]} />
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                      </div>
